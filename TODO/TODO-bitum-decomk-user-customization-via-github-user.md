# TODO-bitum: decomk: user customization via GITHUB_USER
...
Affects: `TODO/TODO-bitum-decomk-user-customization-via-github-user.md`, stage-0 lifecycle templates, selftest lifecycle assertions.
...
Affects: `TODO/TODO-bitum-decomk-user-customization-via-github-user.md`, stage-0 templates, selftests, README lifecycle guidance.
Supersedes: DI-vubol

ID: DI-lumos
Date: 2026-04-19 11:52:09
Status: active
Decision: Make the per-user bootstrap helper non-optional for TODO-bitum and explicitly choose its placement as a required design decision.
...
Affects: `TODO/TODO-bitum-decomk-user-customization-via-github-user.md`, TODO-bitum subtasks, user customization design docs.
Supersedes: DI-pojop
...
TODO-bitum requires a helper mechanism for per-user bootstrap behavior, executed
...
- `GITHUB_USER -> USER` fallback is deferred and is not part of TODO-bitum implementation.
- A decomk-owned dotfiles engine (clone + script dispatch) is out of scope for this TODO.

## Superseded subtasks (overlay track)

- [x] bitum.1 Add a user config search path (`<DECOMK_HOME>/user/decomk.conf`). (Superseded by DI-pojop; not implemented)
- [x] bitum.2 Add `-user-config` flag + `DECOMK_USER_CONFIG` env override. (Superseded by DI-pojop; not implemented)
- [x] bitum.3 Update `loadDefs` to merge config repo + user overlay + explicit config. (Superseded by DI-pojop; not implemented)
- [x] bitum.4 Extend workspace context selection to optionally include `user/<GITHUB_USER>`. (Superseded by DI-pojop; not implemented)
- [x] bitum.5 Document the feature in `README.md` (layout, precedence, examples). (Superseded by DI-pojop; not implemented)
- [x] bitum.6 Add unit tests for layering + key selection (no sudo/network). (Superseded by DI-pojop; not implemented)

## Active subtasks

- [ ] bitum.7 Document the platform-first boundary in `README.md` and `doc/decomk-design.md` (platform dotfiles for personal setup; decomk for policy/make orchestration).
- [ ] bitum.8 Document phase responsibilities for user customization (`updateContent` shared/prebuild-safe; `postCreate` runtime/user-phase).
- [ ] bitum.9 Add/adjust selftests to assert phase separation and that user-phase targets run only in `postCreate`.
- [ ] bitum.10 Decide helper placement (decomk subcommand vs separate decomk-repo binary vs conf-repo artifact vs other) and lock the decision in a DI entry with rationale.
- [ ] bitum.11 Define the non-optional make-invoked helper contract for per-user bootstrap behavior (inputs, outputs, stamps, logs, failure handling).
- [ ] bitum.12 Cross-link this TODO from `TODO/TODO-jirin-decomk-devcontainer-tool-bootstrap.md` and related lifecycle TODOs as the user-customization contract.
