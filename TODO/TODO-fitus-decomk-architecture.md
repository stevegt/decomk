# TODO-fitus: decomk: software architecture
...
This TODO is a design doc. It should stay aligned with `TODO-jirin`
...
- Note: this layout assumes the “make-as-engine” direction discussed in
  TODO-gazot. If we later replace `make` as the execution/state engine,
...
- [x] fitus.1 Decide default persistent directory policy (`/var/decomk` by default; `DECOMK_HOME` override) and document it.
- [ ] fitus.2 Specify `decomk.conf` grammar + search/merge precedence (config repo vs explicit `-config`).
- [ ] fitus.3 Define workspace discovery + context-key selection (git origin + basename fallbacks; merged multi-workspace expansion).
- [ ] fitus.4 Specify tokenization/quoting rules (single quotes) and how they map to `exec.Command` argv (no shell).
- [ ] fitus.5 Specify macro expansion semantics (isconf-like; add cycle detection + max depth).
- [ ] fitus.6 Specify stamp directory conventions (file targets, optional pre-touch, and clean/force behaviors).
- [ ] fitus.7 Specify env export file format + stable path (`.../env.sh`).
- [ ] fitus.8 Specify per-run log format + file set written per run.
- [ ] fitus.9 Confirm package layout fits repo conventions (no `internal/`/`pkg/`; minimal deps).
- [ ] fitus.10 Specify config repo update behavior (clone/pull, offline mode, and failure policy).
