# TODO-gazot: decomk: state engine (make vs alternatives) + lunamake review
...
- [ ] gazot.1 Clarify the domain: do we want DAG semantics or “ordered run-once” semantics?
- [ ] gazot.2 Confirm whether `decomk` should default to “touch all stamps first” (current leaning: yes).
- [ ] gazot.3 Document a recommended Makefile style for decomk (scripts for complex steps; stamp helpers).
- [ ] gazot.4 Prototype a minimal journal/stanza runner (no deps) to validate the `.lm` idea.
- [ ] gazot.5 Decide whether lunamake is a predecessor we port from, or a separate project we leave alone.
- [ ] gazot.6 If adopting `.lm`, define a tiny grammar + state journal format (JSONL or sqlite).
- [ ] gazot.7 Update TODO-fitus if this decision changes the architecture.
