# TODO-takoh: decomk: align flow with isconf (identity → vars → PACKAGES → make)
...
- [x] takoh.1 Document isconf flow precisely (with examples and invariants).
- [x] takoh.2 Define decomk’s positional-args semantics (`INSTALL` vars vs literal targets).
- [x] takoh.3 Decide default action when no args provided (what does isconf do?)
- [x] takoh.4 Update `README.md` to describe the isconf-aligned flow + examples.
- [x] takoh.5 Implement action-arg parsing (derive targets from effective tuple values).
- [x] takoh.6 Add tests for action-arg parsing and fallback behavior.
