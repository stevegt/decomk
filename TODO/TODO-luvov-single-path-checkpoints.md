# TODO-luvov: single-path checkpoints
...
Affects: `doc/image-management.md`, `TODO/TODO-luvov-single-path-checkpoints.md`, `TODO/TODO-topan-codespaces-block-prebuild-profiles.md`, `TODO/TODO-fuviv-devpod-gcp-selfhost-migration.md`, `TODO/TODO.md`.

ID: DI-palak
Date: 2026-04-18 11:54:06
Status: active
Decision: Implement TODO luvov.3 as a first-class `decomk checkpoint` subcommand (not an external script), using existing decomk CLI structure (`main.go` switch + `flag.FlagSet`) and running prebuild/common lifecycle via `devcontainer up --prebuild` before checkpointing with Docker.
...
Constraints: Keep TODO luvov.1/luvov.2 deferred; require caller-provided `--tag`; emit JSON to stdout; default to removing the container after checkpoint; no Cobra migration in this phase.
Affects: `cmd/decomk/main.go`, `cmd/decomk/checkpoint.go`, `cmd/decomk/checkpoint_test.go`, `README.md`, `TODO/TODO-luvov-single-path-checkpoints.md`.

ID: DI-gukuz
Date: 2026-04-19 12:48:30
Status: active
Decision: Drive TODO-luvov execution with plain-English user stories for both DevOps engineer and Dev team member personas; defer Gherkin conversion.
...
Affects: `TODO/TODO-luvov-single-path-checkpoints.md`, `TODO/TODO-topan-codespaces-block-prebuild-profiles.md`.

ID: DI-tavaf
Date: 2026-04-19 13:01:12
Status: active
Decision: Replace short user-story bullets with day-in-the-life narratives for both DevOps engineer and dev team member personas in TODO-luvov.
...
Affects: `TODO/TODO-luvov-single-path-checkpoints.md`.
Supersedes: DI-gukuz

ID: DI-zitas
Date: 2026-04-19 14:18:22
Status: active
Decision: Reframe TODO-luvov around a speed-only objective and single-path checkpoints: use the same `updateContent -> decomk run` path for both prebuilds and checkpoint image builds, and remove parity-comparator/proof-model work from active scope.
...
Constraints: Preserve checkpoint v1 command shape, preserve phase split (`updateContent` included, `postCreate` excluded), avoid introducing new speed-metric gates, and keep this TODO path renamed to `TODO/TODO-luvov-single-path-checkpoints.md`.
Affects: `TODO/TODO-luvov-single-path-checkpoints.md`, `TODO/TODO-topan-codespaces-block-prebuild-profiles.md`, `doc/image-management.md`, `TODO/TODO.md`.
Supersedes: DI-tavaf
...
Affects: `TODO/TODO-luvov-single-path-checkpoints.md`, `TODO/TODO-topan-codespaces-block-prebuild-profiles.md`, `doc/image-management.md`, `README.md`.
Supersedes: DI-zitas
...
Intent: Deliver TODO luvov.3 as a usable operator workflow now, with machine-readable artifacts for external release/test gates and explicit safeguards against accidental channel-tag overwrite.
...
Status: active
Decision: Complete TODO luvov.7 as documentation-only by defining a canonical operator/CI handoff contract (narrative + command templates + artifact contract) for `build -> push -> external test -> tag`, and explicitly defer luvov.4, luvov.6, and luvov.8.
...
Constraints: Keep external test gate outside decomk command execution; keep luvov.4/luvov.6/luvov.8 open and deferred; keep command contract aligned with implemented checkpoint JSON outputs.
Affects: `TODO/TODO-luvov-single-path-checkpoints.md`, `doc/image-management.md`, `README.md`.

ID: DI-matar
Date: 2026-04-20 21:41:06
Status: active
Decision: Defer TODO luvov.5 deterministic pinning checks in the current phase, alongside already-deferred luvov.4/luvov.6/luvov.8.
...
Constraints: Leave luvov.5 open and explicitly labeled deferred; do not alter the existing checkpoint command contract.
Affects: `TODO/TODO-luvov-single-path-checkpoints.md`.

ID: DI-rahip
Date: 2026-04-20 21:48:41
Status: active
Decision: Define luvov.1 as a documented canonical single-path contract now, and replace luvov.2's old block-base wording with channel/pin progression (`candidate -> immutable+testing/unstable -> external test -> stable move`) as the active rollout method.
Intent: Remove ambiguity about checkpoint progression semantics and align TODO-luvov with the implemented `build/push/tag` contract plus the canonical consumer selector model from TODO-topan.
Constraints: Keep luvov.2 as planning work (not yet complete), keep deferred items luvov.4/luvov.5/luvov.6/luvov.8 unchanged, and avoid introducing new command behavior in this step.
Affects: `TODO/TODO-luvov-single-path-checkpoints.md`.

ID: DI-nazof
Date: 2026-04-20 21:51:13
Status: active
Decision: Close TODO luvov.2 by adopting channel/pin progression as the canonical checkpoint rollout method and recording explicit operator handoff points for each stage.
Intent: Finalize progression semantics so TODO-luvov has no remaining ambiguity between old block-base phrasing and the current `build/push/tag` rollout model.
Constraints: Keep deferred scope unchanged for luvov.4/luvov.5/luvov.6/luvov.8; avoid adding new command behavior in this pass.
Affects: `TODO/TODO-luvov-single-path-checkpoints.md`.
Supersedes: DI-rahip
...
Affects: `cmd/decomk/checkpoint.go`, `cmd/decomk/checkpoint_test.go`, `README.md`, `TODO/TODO-luvov-single-path-checkpoints.md`.
...
- GCP rollout execution details (tracked in TODO-fuviv).

## Dependencies and links

- Lifecycle evidence baseline: `TODO/TODO-dahuk-phase-eval-lifecycle-spike.md`
- Block profile selection model: `TODO/TODO-topan-codespaces-block-prebuild-profiles.md`
- Self-host migration track: `TODO/TODO-fuviv-devpod-gcp-selfhost-migration.md`
- Apt package retention strategy: `TODO/TODO-hunit-apt-package-retention-strategy.md`

## Subtasks

- [x] luvov.1 Define canonical single-path checkpoint contract (prebuild + checkpoint both execute the same `updateContent -> decomk run` flow with identical input surfaces, and promotion always follows `build -> push -> external test -> tag`).
- [x] luvov.2 Define progression workflow as channel/pin rollout (`candidate -> immutable + testing/unstable -> external test -> stable move`, with consumer repos either following channel tags or pinning immutable tags) and document operator handoff points.
- [x] luvov.3 Implement checkpoint command family (`build`, `push`, `tag`) with explicit release gate between push and stable tagging.
- [x] luvov.3.1 Add checkpoint subcommand routing in `cmd/decomk/main.go` and usage text for `checkpoint build`, `checkpoint push`, and `checkpoint tag`.
- [x] luvov.3.2 Implement `checkpoint build` handler to run prebuild/common lifecycle (`devcontainer up --prebuild`), commit local candidate image, and emit machine-readable JSON output.
- [x] luvov.3.3 Implement `checkpoint push [-m] <source> <tag...>` using positional args (source digest/ref/image ID + one or more destination tags).
- [x] luvov.3.4 Implement `checkpoint tag [-m] <source> <tag...>` for retagging tested remote candidates to channel tags (including `stable`) without rebuild.
- [x] luvov.3.5 Enforce default fail-on-existing-tag behavior; require explicit `-m` to move existing channel tags.
- [x] luvov.3.6 Include source-resolution and tag results in JSON output so external test/release tooling can consume build/push/tag artifacts.
- [x] luvov.3.7 Keep temporary checkpoint container cleanup explicit for `build` (`--keep-container` for diagnostics, default cleanup otherwise).
- [x] luvov.3.8 Add focused unit tests for build/push/tag success and failure paths (source resolution errors, tag collision without `-m`, registry/tag move errors, cleanup behavior).
- [ ] luvov.4 Enforce phase separation so checkpoint images exclude runtime/user-phase (`postCreate`) side effects. (Deferred in this pass; see DI-bavis.)
- [ ] luvov.5 Add deterministic pinning checks (base digest, package/tool versions, git refs, and other mutable inputs) so checkpoint behavior remains stable. (Deferred in this pass; see DI-matar and TODO-hunit for apt package retention dependency.)
- [ ] luvov.6 Add same-path verification evidence capture (lifecycle markers/command traces) to prove checkpoints were created through the shared `updateContent` flow. (Deferred in this pass; see DI-bavis.)
- [x] luvov.7 Add repeatable operator/CI entrypoints and documentation for `build -> push -> external test -> tag` block handoff.
- [ ] luvov.8 Execute acceptance runs and record evidence that checkpointed block progression removes repeated setup work from subsequent prebuild/first-boot paths. (Deferred in this pass; see DI-bavis.)

## luvov.3 command contract
...
## luvov.1 canonical single-path contract
...
This closes luvov.1 as a documentation-defined contract baseline.

## luvov.2 progression method (channel/pin)
...
This closes luvov.2 as a documentation-defined rollout progression contract.
...
- `luvov.1` -> `DITL-011-DEVOPS-01`, `DITL-011-DEVTEAM-01`
- `luvov.2` -> `DITL-011-DEVOPS-01`, `DITL-011-DEVTEAM-01`
- `luvov.3` -> `DITL-011-DEVOPS-01`, `DITL-011-DEVTEAM-01`
- `luvov.3.1` -> `DITL-011-DEVOPS-01`
- `luvov.3.2` -> `DITL-011-DEVOPS-01`
- `luvov.3.3` -> `DITL-011-DEVOPS-01`
- `luvov.3.4` -> `DITL-011-DEVOPS-01`
- `luvov.3.5` -> `DITL-011-DEVOPS-01`
- `luvov.3.6` -> `DITL-011-DEVOPS-01`, `DITL-011-DEVTEAM-01`
- `luvov.3.7` -> `DITL-011-DEVOPS-01`
- `luvov.3.8` -> `DITL-011-DEVOPS-01`
- `luvov.4` -> `DITL-011-DEVOPS-01`, `DITL-011-DEVTEAM-01`
- `luvov.5` -> `DITL-011-DEVOPS-01`
- `luvov.6` -> `DITL-011-DEVOPS-01`, `DITL-011-DEVTEAM-01`
- `luvov.7` -> `DITL-011-DEVOPS-01`, `DITL-011-DEVTEAM-01`
- `luvov.8` -> `DITL-011-DEVOPS-01`, `DITL-011-DEVTEAM-01`
