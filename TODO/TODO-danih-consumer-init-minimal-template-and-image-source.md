# TODO-danih: Consumer `decomk init` minimal template + image-source prompt rework
...
- `README.md`
- `TODO/TODO-jirin-decomk-devcontainer-tool-bootstrap.md`
Supersedes:
- DI-tosuk (legacy-flag guidance wording and usage-output presentation details only)

ID: DI-zajub
Date: 2026-05-15 08:28:32 -0700
Status: active
Decision: Treat a consumer repo's selected checkpoint image as the authority for embedded decomk lifecycle metadata, including stage-0 hook commands and the image metadata values for `DECOMK_TOOL_URI` and `DECOMK_CONF_URI`.
Intent: Make the coupling created by minimal consumer `devcontainer.json` output explicit: consumers select an image, and that image carries both filesystem state and the decomk bootstrap policy needed to run it, keeping consumer repos low-churn and avoiding duplicated tool/conf/hook settings in every repo.
Constraints:
- Consumer `devcontainer.json` remains minimal (`name` + `image`) under this TODO.
- Consumer maintainers choose between channel-following and immutable-pinned image selectors; selecting an image also selects its embedded tool/conf channels unless a future DI adds an explicit override mechanism.
- Producer/checkpoint images must keep `devcontainer.metadata` complete enough for devcontainer runtimes to run `updateContentCommand` and `postCreateCommand`.
- The tradeoff is deliberate coupling: consumers gain low churn and centralized promotion, but per-consumer tool/conf channel overrides are not part of the current contract.
Affects:
...
- `cmd/decomk/init_test.go`
- `README.md`
- `TODO/TODO-jirin-decomk-devcontainer-tool-bootstrap.md` (DI append)
- `TODO/TODO.md`
- `docs/thought-experiments/TE-baras-consumer-init-image-source.md` (new)
- `docs/thought-experiments/TE-jajov-template-architecture.md` (new)
Supersedes:
- DI-zofav (consumer template/default behavior portions only)
- DI-zopat (consumer prompt/default/import portions only)
...
1. `docs/thought-experiments/TE-baras-consumer-init-image-source.md`
...
2. `docs/thought-experiments/TE-jajov-template-architecture.md`
...
- [x] danih.1 Add this TODO to `TODO/TODO.md` in priority order.
- [x] danih.2 Append DI entries to `TODO/TODO-jirin-decomk-devcontainer-tool-bootstrap.md` referencing this TODO.
- [x] danih.3 Add TE doc `TE-baras-consumer-init-image-source.md`.
- [x] danih.4 Add TE doc `TE-jajov-template-architecture.md`.
- [x] danih.5 Add new consumer template file `cmd/decomk/templates/consumer.devcontainer.json.tmpl`.
- [x] danih.6 Embed consumer template in `cmd/decomk/init_templates.go`.
- [x] danih.7 Implement consumer minimal render path in `cmd/decomk/init.go`.
- [x] danih.8 Add `-conf-url` flag and parser in `cmd/decomk/init.go`.
- [x] danih.9 Add consumer legacy-flag misuse validation in `cmd/decomk/init.go`.
- [x] danih.10 Implement interactive source menu flow in `cmd/decomk/init.go`.
- [x] danih.11 Implement keep-existing-image menu option behavior.
- [x] danih.12 Implement conf-url derivation via clone + ref checkout + parse.
- [x] danih.13 Upgrade JSONC comment handling for derivation parser.
- [x] danih.14 Implement non-interactive precedence/failure behavior exactly per locks.
- [x] danih.15 Ensure `-image` short-circuits conf-url prompting.
- [x] danih.16 Keep consumer stage0 script generation unchanged.
- [x] danih.17 Update consumer init tests in `cmd/decomk/init_test.go`.
- [x] danih.18 Update `README.md` for new consumer UX.
- [x] danih.19 Run `go test ./...`.
- [x] danih.20 Run `errcheck ./...`.
- [x] danih.21 Perform comment-delta audit on touched code files.
- [ ] danih.22 Prepare Decision Matrix + Runtime Path Touch Matrix in final handoff.
