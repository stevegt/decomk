# TODO-hukuv: mob-sandbox prebuild `/var/*` permissions via checkpoint image
...
- [ ] hukuv.1 [repo: decomk-conf-cswg] Verify producer Dockerfile ownership contract exists and is committed.
  - Confirm `.devcontainer/Dockerfile` includes:
    - `mkdir -p /var/decomk /var/log/decomk`
    - `chown -R vscode:vscode /var/decomk /var/log/decomk`

- [ ] hukuv.2 [repo: decomk-conf-cswg] Build a local checkpoint candidate image from producer workspace.
  - Run:
...
- [ ] hukuv.3 [repo: decomk-conf-cswg] Publish candidate to immutable + testing tags.
  - Run:
...
- [ ] hukuv.4 [repo: mob-sandbox] Update consumer devcontainer image selector to checkpoint testing tag.
...
- [ ] hukuv.5 [repo: mob-sandbox] Re-run prebuild and verify permission failures are eliminated.
...
- [ ] hukuv.6 [repo: mob-sandbox] After testing-tag validation, switch to stable-channel policy as needed.
  - Preferred production target: `"image": "ghcr.io/ciwg/decomk-conf-cswg:stable"`.

- [ ] hukuv.7 [repo: decomk-conf-cswg] Promote tested immutable image to stable.
  - Run:
    - `decomk checkpoint tag -m ghcr.io/ciwg/decomk-conf-cswg:block00-<YYYYMMDD> ghcr.io/ciwg/decomk-conf-cswg:stable`

- [ ] hukuv.8 [repo: decomk] Record evidence links and cross-reference TODO-luvov rollout contract.
  - Add artifact paths/command outputs to this TODO.
  - Link to `TODO/TODO-luvov-single-path-checkpoints.md` operator flow where relevant.
