# TODO-jirin: decomk: isconf-like devcontainer tool bootstrap
...
Affects: `stage0/stage0.go`, `cmd/decomk/templates/decomk-stage0.sh.tmpl`, generated stage-0 examples, `scripts/release.sh`, `Makefile`, `README.md`, `TODO/TODO-dobup-mob-sandbox-pilot.md`, `TODO/TODO-jirin-decomk-devcontainer-tool-bootstrap.md`.
Supersedes: DI-linop
...
Affects: `cmd/decomk/main.go`, `cmd/decomk/main_test.go`, `TODO/TODO-jirin-decomk-devcontainer-tool-bootstrap.md`.
...
Constraints: No migration guidance text (pre-production); consumer misuse check order is deterministic (`conf-uri`, `tool-uri`, `home`, `log-dir`, `fail-no-boot`); producer mode behavior remains unchanged unless explicitly scoped by TODO-danih.
Affects: `cmd/decomk/init.go`, `cmd/decomk/init_templates.go`, `cmd/decomk/templates/consumer.devcontainer.json.tmpl`, `cmd/decomk/init_test.go`, `README.md`, `TODO/TODO-danih-consumer-init-minimal-template-and-image-source.md`.
Supersedes: DI-zopat (consumer config-source and prompt behavior details only), DI-bonop (consumer output-shape details only)
...
Supersedes: DI-zopat (producer Dockerfile default-image/default-source details)
...
Supersedes: DI-lafib (consumer conf identity fetch path), DI-bonop (consumer identity flow details), DI-lobin (remoteUser/containerUser output contract)
...
Supersedes: DI-bonop (identity variable naming only), DI-kutod (computed variable naming only)
...
Affects: `cmd/decomk/main.go`, `cmd/decomk/init.go`, `cmd/decomk/init_conf.go`, `stage0/stage0.go`, `confrepo/confrepo.go`, `cmd/decomk/templates/*`, generated examples, `README.md`, `doc/decomk-design.md`, `TODO/TODO-rufiz-conf-repo-init-scaffolding.md`.
Supersedes: DI-votit (command shape only), DI-lobin (identity source/flow details)
...
Affects: `cmd/decomk/init.go`, `cmd/decomk/init_test.go`, `cmd/decomk/templates/devcontainer.json.tmpl`, `cmd/decomk/templates/postCreateCommand.sh.tmpl`, generated `examples/devcontainer/*`, generated selftest workspace templates, `README.md`, `TODO/TODO-jirin-decomk-devcontainer-tool-bootstrap.md`.
Supersedes: DI-votit
...
Status: active
Decision: Remove TODO subtasks `jirin.6` and `jirin.7` from TODO-jirin because they are no longer valid work items.
Intent: Keep TODO-jirin aligned with current scope and avoid carrying obsolete planning items that create false backlog.
Constraints: Preserve stable TODO identities for remaining items and avoid renaming other TODO entries.
Affects: `TODO/TODO-jirin-decomk-devcontainer-tool-bootstrap.md`.

ID: DI-pabul
Date: 2026-04-21 22:18:34
Status: active
Decision: Track TODO `jirin.8` execution details in a dedicated cross-repo checklist file (`TODO/TODO-dobup-mob-sandbox-pilot.md`) instead of embedding those details in TODO-jirin.
Intent: Keep TODO-jirin focused on decomk bootstrap scope while giving the mob-sandbox pilot a decision-complete, repo-scoped execution checklist.
Constraints: Keep TODO handles stable, keep `jirin.8` open until TODO-dobup completion criteria and evidence are satisfied, and label execution tasks by repository.
Affects: `TODO/TODO-jirin-decomk-devcontainer-tool-bootstrap.md`, `TODO/TODO-dobup-mob-sandbox-pilot.md`, `TODO/TODO.md`.

ID: DI-jasoh
Date: 2026-04-22 11:06:00
Status: active
Decision: Track shared config repo scaffolding as a dedicated follow-on workstream (`TODO/TODO-rufiz-conf-repo-init-scaffolding.md`) and reference it from TODO-jirin via a new subtask.
Intent: Keep TODO-jirin aligned with the expanded bootstrap contract where conf repos can be initialized with decomk, without overloading existing subtask scope.
Constraints: Preserve stable TODO handles and keep the conf-repo-init implementation details centralized in TODO-rufiz.
Affects: `TODO/TODO-jirin-decomk-devcontainer-tool-bootstrap.md`, `TODO/TODO-rufiz-conf-repo-init-scaffolding.md`, `TODO/TODO.md`.
...
Affects: `VERSION`, `cmd/versiongen/main.go`, `cmd/decomk/generate_version.go`, `cmd/decomk/version_generated.go`, `cmd/decomk/main.go`, `scripts/release.sh`, `Makefile`, `README.md`, `TODO/TODO-jirin-decomk-devcontainer-tool-bootstrap.md`.
...
Affects: `stage0/stage0.go`, `cmd/decomk/templates/devcontainer.json.tmpl`, `cmd/decomk/init.go`, `cmd/decomk/init_test.go`, `README.md`, `TODO/TODO-jirin-decomk-devcontainer-tool-bootstrap.md`.
...
- [x] jirin.1 Decide naming for the `hosts.conf` analog (`decomk.conf` + optional `decomk.d/*.conf`).
- [x] jirin.2 Choose config file name/location and syntax (use isconf-like `decomk.conf` grammar).
- [x] jirin.3 Choose wrapper language (Go vs Bash) and document the tradeoffs/decision.
- [x] jirin.4 Implement macro expansion (isconf `expandmacro` semantics) without Perl.
- [x] jirin.5 Implement env export file generation and decide where it is written.
- [ ] jirin.8 Execute the `mob-sandbox` pilot and generalize outcomes, tracked in `TODO/TODO-dobup-mob-sandbox-pilot.md`.
- [x] jirin.9 Add a reference `examples/devcontainer/postCreateCommand.sh` that performs stage-0 bootstrap (ensure decomk in `PATH`, sync config repo, run decomk), then reuse it in pilots.
- [x] jirin.10 Add `decomk init` to scaffold `.devcontainer` files from embedded templates with flag/prompt inputs.
- [x] jirin.11 Generate example scaffold files from canonical templates and enforce sync via `go generate`/tests.
- [x] jirin.12 Make the production example devcontainer standalone by including a Dockerfile build entry and companion Dockerfile.
- [ ] jirin.13 Add optional derived `env.mk` output generated from the same canonical env tuple sequence as `env.sh` (without making `env.mk` the source of truth).
- [x] jirin.14 Add strict non-overwrite defaults to `decomk init` (fail unless `-f`/`-force` when target files exist) and provide explicit commit/force/difftool reconciliation guidance.
- [x] jirin.15 Add stage-0 template ownership banners and README onboarding reorganization, plus a canonical legacy-variable migration mapping section.
- [x] jirin.16 Add first-class shared conf repo scaffolding (`decomk init -conf`), tracked in `TODO/TODO-rufiz-conf-repo-init-scaffolding.md`.
- [x] jirin.17 Add source-controlled release versioning (`VERSION` + generated version file) and a `make release-minor` workflow.
- [x] jirin.18 Add image fallback rendering for non-build init scaffolds and reuse existing devcontainer values as defaults during `decomk init -f`.
