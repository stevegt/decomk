# TODO-hunit: apt package retention strategy
...
Constraints: This TODO is planning-only in the current pass (no production rollout yet), must produce a decision-complete selection gate, and must feed TODO-luvov deterministic pinning work.
Affects: `TODO/TODO-hunit-apt-package-retention-strategy.md`, `TODO/TODO-luvov-single-path-checkpoints.md`, `TODO/TODO.md`, future image-management implementation tasks.
...
- [ ] hunit.1 Capture at least two concrete failures where pinned apt versions were no longer available upstream.
- [ ] hunit.2 Define acceptance criteria for “rebuildable” (time window, artifact guarantees, and verification method).
- [ ] hunit.3 Prototype and document a managed mirror/snapshot workflow against one decomk checkpoint build.
- [ ] hunit.4 Prototype and document `~/lab/apt-tar/` resurrection against the same build path.
- [ ] hunit.5 Compare both options against the criteria and record final DI decision with rationale and constraints.
- [ ] hunit.6 Write implementation handoff for the chosen path, including required repo changes and rollout sequence.
...
- A final decision gate exists (`hunit.5`) before implementation work begins.
- TODO-luvov deterministic pinning work references this TODO as dependency.
