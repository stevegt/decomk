# TODO-mirut: decomk: log dir override + naming cleanup
...
Affects: `cmd/decomk/main.go`, `cmd/decomk/main_test.go`, `TODO/TODO-mirut-decomk-log-dir-and-terminology.md`.
...
Affects: `cmd/decomk/main.go`, `state/*`, `README.md`, `TODO/TODO-fitus-decomk-architecture.md`, unit tests for log root selection/fallback.
...
6. **Docs**
   ...
   - `TODO/TODO-fitus-decomk-architecture.md`:
     - replace “audit” wording with “log”
     - document the same default/override/fallback policy as README

## Subtasks
- [x] mirut.1 Rename “audit” identifiers/comments to “log” (cmd + state + docs).
- [x] mirut.2 Add `-log-dir` flag and `DECOMK_LOG_DIR` env var (absolute-only).
- [x] mirut.3 Resolve/store `LogRoot` + `LogRootExplicit` in `resolvedPlan`.
- [x] mirut.4 Implement per-run log dir creation with default-only fallback to `<home>/log`.
- [x] mirut.5 Remove/replace `state.AuditDir`; restore `state.LogDir(home)` to `<home>/log`; keep `state.DefaultLogDir`.
- [x] mirut.6 Add unit tests for precedence + fallback behavior.
- [x] mirut.7 Update `README.md` and `TODO/TODO-fitus-decomk-architecture.md` for consistency + fix tab-indented bullets.
- [x] mirut.8 Run `gofmt` + `go test ./...`.
- [x] mirut.9 Replace fixed `98-decomk` run-summary path with phase mapping via `DECOMK_MOTD_PHASES` + `DECOMK_STAGE0_PHASE`.
- [x] mirut.10 Add unit tests for mapping parse/skip/error behavior and mirrored fallback filenames.
- [x] mirut.11 Document `DECOMK_MOTD_PHASES` phase-file mapping in `README.md` and `doc/decomk-design.md`.
- [x] mirut.12 Also write mapped `version` MOTD file when `DECOMK_MOTD_PHASES` includes `NN:version`.
- [x] mirut.13 Add tests that verify `version` MOTD write and fallback behavior.
- [x] mirut.14 Update version MOTD format: leading blank line before `decomk version:`, remove `runtime phase:` line.
